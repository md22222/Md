...
setInterval(() => {
    http.get(`http://darker11.glitch.me/`); /// حط اسم المشروع تبعك name تعديل مهم بدل
}, 280000);
...
client.on("message", async message => {
    ...
    if (message.content.startsWith(prefix + "bc")) {
        ...
        let args = message.content
            ...
            .slice(1)
            .join(" ");
        if (!message.author.id === "524323178105798656") return; ///تعديل مهم حط الايدي تبعك
...
client.on("ready", () => {
    client.user.setActivity(`Md.`, { ///تعديل مهم حالة البوت
        type: "WATCHING"
    });Md ///تعديل حالة البوت
});
